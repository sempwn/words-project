...
window.newURL = (name,desc,url) ->
  location.replace("#" + encodeURIComponent(name))
  
  d3.select('#word-desc').html(desc)
  d3.select('#word-link').html(name).attr('href',url)
  d3.selectAll('#vis').remove()
  d3.select('#chart-container').append('div').attr('id','vis')
...
click = (d) ->
  ...
  d3.select('#word-desc').html(d.desc)
  d3.select('#word-link').html(d.name).attr('href',d.url)
  d3.selectAll('#vis').remove()
  d3.select('#chart-container').append('div').attr('id','vis')
  # create a new Bubbles chart
  plot = Bubbles()
...
$ ->
  ...
  # load our data
  d3.json("home/index.json", display)